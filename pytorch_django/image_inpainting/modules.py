...
class GatedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding='auto', dilation=1, activation=nn.ELU()):
        super().__init__()

        padding = dilation*(kernel_size-1)//2 if padding == 'auto' else padding
        self.activation = activation
        self.out_channels = out_channels
        num_out_channels = out_channels if self.out_channels == 3 or self.activation is None else 2*out_channels

        # Initialize the convolution layers
        self.conv2d = nn.Conv2d(in_channels, num_out_channels, kernel_size, stride, padding=padding, dilation=dilation, bias=True)
        ...
        _init_conv_layer(self.conv2d, self.activation)

    def forward(self, x):
        x = self.conv2d(x)
        if self.activation is None or self.out_channels == 3:
            return x
        feature, mask = torch.split(x, self.out_channels, dim=1)
        feature = self.activation(feature)
        mask = self.sigmoid(mask)
        x = feature * mask
        return x

class TransposedGatedConv2d(nn.Module):
	def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, scale_factor=2, interpolate_mode='nearest'):
		super().__init__()
		...
		if interpolate_mode == 'nearest':
			...
			raise Exception(f"Unsupported interpolate mode: {mode}")
...
class DConv(nn.Module):
    def __init__(self, cnum_in,
        ...
        self.conv_sn = SpectralNormConv2d(cnum_in, cnum_out, ksize, stride, padding)
...
class ContextualAttention(nn.Module):
    ...
    def forward(self, f, b, mask=None):
        ...
        f = torch.nn.functional.interpolate(f, scale_factor=1./self.rate,
                          mode='nearest', recompute_scale_factor=False)
        b = torch.nn.functional.interpolate(b, scale_factor=1./self.rate,
                          mode='nearest', recompute_scale_factor=False)
